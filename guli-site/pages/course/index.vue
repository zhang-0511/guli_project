              ...
              <li v-for="item in data.items" :key="item.id">
                <div class="cc-l-wrap">
                  <section class="course-img">
                    <img :src="item.cover" class="img-responsive" style="width: 267px;height: 150px;" :alt="item.title">
                    <div class="cc-mask">
...
//查询课程列表
    initCourse(){
      courseApi.pageCourseFront(1, 8,this.courseQueryVo).then(response => {
    ...
    searchBuyCount() {
      ...
      this.gmtCreateSort = "";
      this.priceSort = "";
      this.courseQueryVo.buyCountSort = this.buyCountSort;
      this.courseQueryVo.gmtCreateSort = this.gmtCreateSort;
      this.courseQueryVo.priceSort = this.priceSort;
      this.gotoPage(this.page)
    },
    //更新时间查询
    searchGmtCreate() {
    ...
    gotoPage(page) {
      this.page = page
      courseApi.pageCourseFront(page, 8, this.courseQueryVo).then(response => {
        this.data = response.data.data
      }).catch(error=>{this.$message.error(error.message);})
    }
  },
