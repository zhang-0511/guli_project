      ...
      <form name="flowForm" id="flowForm" method="post" action="">
          ...
          <tr class="good">
            <td class="name First">
              <a target="_blank" :href="'/course/'+order.courseId">
                <img :src="order.courseCover" alt=""></a>
              <div class="goodInfo">
        ...
        <div class="Finish">
          ...
          <div class="Main fl">
            ...
            <div class="fr">
              <p>
                共 <strong class="red">1</strong> 件商品，合计<span class="red f20">￥<strong id="AllPrice">{{order.totalFee}}</strong></span>
              </p>
            </div>
          </div>
...
export default {
  ...
  asyncData({params,error}) {
    return orderApi.getOrderInfo(params.orderId).then(res=>{
      if (res.data.success === true){
        return {order: res.data.data.order}
      }
    }).catch(error=>{this.$message.error(error.message);})
  },
  methods:{
    toPay(){
      this.$router.push({name:'pay-info',params:{orderId : this.order.orderNo}})
    }
  }
}
</script>
