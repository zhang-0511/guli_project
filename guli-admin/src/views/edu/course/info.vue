<template>
  <div class="app-container">
    ...
    <el-steps align-center process-status="wait" style="margin-bottom: 40px"  :active="1" finish-status="finish">
      <el-step :title="title1" icon="el-icon-edit" description="填写课程的基本信息和表述信息"></el-step>
    ...
    <el-form label-width="120px" :model="courseInfo" :rules="rules" ref="courseInfo">
      ...
      <el-form-item>
        ...
        <el-button :disabled="saveBtnDisabled" type="primary" style="margin-top: 12px;" @click="saveOrUpdate('courseInfo')">保存并下一步</el-button>
      </el-form-item>
    </el-form>
  ...
  methods:{
    init(){
      if(this.$route.params && this.$route.params.id){
        ...
        this.fileUrl = process.env.BASE_API + '/oss/fileOss/coverUpload'
    ...
    saveCourse(){
      ...
      this.courseInfo.subjectId = this.courseInfo.cascade[1]
       course.saveCourseInfo(this.courseInfo)
         ...
         }).catch(error=>{this.$message.error('提交失败，请检查信息是否符合要求并重新提交');})
    },
    updateCourse(){
      this.courseInfo.subjectParentId = this.courseInfo.cascade[0]
      this.courseInfo.subjectId = this.courseInfo.cascade[1]
